...
# ==========================================
# REBASING: ALIGNING CUSTOM INDEX WITH CCI IN 2011
# ==========================================
yearly_index_raw = combined_index.groupby(combined_index['Date_Plot'].dt.year)['Custom_Index'].mean()
yearly_cci = combined_index.groupby(combined_index['Date_Plot'].dt.year)['CCI'].mean()

try:
    cci_2011_base = yearly_cci.loc[2011]
    custom_2011_base = yearly_index_raw.loc[2011]
    adjustment_factor = cci_2011_base / custom_2011_base
    yearly_index = yearly_index_raw * adjustment_factor
except KeyError:
    # Fallback if 2011 data is somehow missing
    yearly_index = yearly_index_raw
...
for col in scaled_macro_evolution.columns:
    ...
    for x, y in zip(scaled_macro_evolution.index, scaled_macro_evolution[col]):
        # The FIX: Push 'View' numbers below the line, keep others above
        if col == 'View_Score':
            y_offset = -4
            va_align = 'top'
        else:
        ...
        plt.text(x, y + y_offset, f"{y:.0f}", ha='center', va=va_align, fontsize=10, color=feature_colors[col], fontweight='bold')
...
for x, y in zip(yearly_index.index, yearly_index):
    plt.text(x, y + 6, f"{y:.0f}", ha='center', va='bottom', fontsize=11, color='black', fontweight='bold')
...
if not yearly_cci.isna().all():
    ...
    for x, y in zip(yearly_cci.index, yearly_cci):
        if pd.notna(y):
            ...
            # The FIX: Push CCI numbers below its dotted line to avoid crossing the Custom Index
            plt.text(x, y - (15 if x == 2026 else 8), lbl, ha='center', va='top', fontsize=10, color='gray', fontweight='bold')

plt.ylim(bottom=-10, top=max(yearly_index.max(), yearly_cci.max() if not yearly_cci.isna().all() else 0) * 1.15)
plt.title('Absolute Feature Contribution (Index Points)', fontsize=14, fontweight='bold')
...
ax.set_title('Global Feature Importance Trend (15-Year Market Shift)', fontsize=18, fontweight='bold', pad=15)
...
ax.set_title('The Micro-Psychology: Feature Importance by Estate (15-Year Baseline)', fontsize=18, fontweight='bold', pad=20)
...
def plot_pie_and_stats(ax, data, title, category_order, color_scheme, estate_title=None):
    ...
    wedges, texts, autotexts = ax.pie(sizes, colors=color_scheme, autopct='%1.1f%%', startangle=90, textprops={'color': "w", 'weight': 'bold', 'fontsize': 11}, wedgeprops={'edgecolor': 'w', 'linewidth': 1})
    full_title = f"{estate_title}\n{title}" if estate_title else title
    ax.set_title(full_title, fontsize=14, fontweight='bold', pad=15)
    
    legend_labels = [f"{cat}: {counts.get(cat, 0):,} ({pcts.get(cat, 0):.1f}%)" for cat in category_order]
    
    # Legend formatting fix for Estate Titles
    legend = ax.legend(wedges, legend_labels, title=estate_title, loc="center left", bbox_to_anchor=(-0.65, 0.5), fontsize=11, framealpha=0.9, edgecolor='gray', handlelength=1.5)
    if legend.get_title():
...
for i, col_name in enumerate(columns):
    ...
    display_name = "Total Market (3 Estates)" if col_name == 'Total Market' else col_name
    
    plot_pie_and_stats(axes[0, i], df_target['Size_Tier'], 'Size Distribution', ['Small', 'Medium', 'Large'], colors_tiers, estate_title=display_name)
    plot_pie_and_stats(axes[1, i], df_target['View_Tier'], 'View Distribution', ['Adverse', 'Standard', 'Premium'], colors_tiers, estate_title=display_name)
    plot_pie_and_stats(axes[2, i], df_target['Floor_Tier'], 'Floor Distribution', ['Low', 'Mid', 'High'], colors_tiers, estate_title=display_name)
    plot_pie_and_stats(axes[3, i], df_target['Distance_Tier'], 'Distance (to MTR) Distribution', ['Close', 'Mid', 'Far'], [color_high, color_mid, color_low], estate_title=display_name)
...
# Calculate statistics BEFORE trimming
print("Applying Outlier Trimming (Removing bottom 2% anomalies by Year, Estate, and Size)...")
estates_list = df['Estate'].unique()
pre_stats = {estate: len(df[df['Estate'] == estate]) for estate in estates_list}

# Trim outliers (bottom 2% by Year, Estate, and Size Tier)
lower_bounds = df.groupby(['Year', 'Estate', 'Size_Tier'])['PSF'].transform(lambda x: x.quantile(0.02))
df = df[df['PSF'] >= lower_bounds].reset_index(drop=True)

# Calculate and print the detailed statistics AFTER trimming
print("\n--- Outlier Trimming Statistics ---")
total_orig = 0
total_rem = 0
total_remain = 0

for estate in estates_list:
    orig = pre_stats[estate]
    remain = len(df[df['Estate'] == estate])
    removed = orig - remain
    
    total_orig += orig
    total_rem += removed
    total_remain += remain
    print(f"{estate:<20} | Original: {orig:>6,} | Removed: {removed:>4,} | Remaining: {remain:>6,}")

print("-" * 65)
print(f"{'TOTAL MARKET':<20} | Original: {total_orig:>6,} | Removed: {total_rem:>4,} | Remaining: {total_remain:>6,}\n")
